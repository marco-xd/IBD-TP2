...
def insertFriends(db):
	...
	for jName in jzimFriends:
		...
		for fnd in friends:
			...
			VALUES ('Friends', %d, %s, %s)" % (friends_since, id1, id2)
			try:
				cursor.execute(sql)
				db.commit()
			except:
...
def insertOwned(db):
	...
	for jName in jzimOwned:
		...
		for gId in gamesId:
			...
			VALUES ('%s', %d, %d, %d)" % (myId, int(gId), playtime_forever, playtime_2weeks)
			try:
				cursor.execute(sql)
				db.commit()
			except:
	...
	for jName in jzimAchievements:
		...
		for gId in gamesId:
			...
			for aux in aux2:
				...
				VALUES ('%s', %d, '%s', 1)" % (myId, int(gId), name)
				try:
					cursor.execute(sql)
					db.commit()
				except:
	...
	for jName in jzimStats:
		...
		for gId in gamesId:
			...
			for aux in aux2:
				...
				VALUES ('%s', %d, '%s', %d1)" % (myId, int(gId), name, value)
				try:
					cursor.execute(sql)
					db.commit()
				except:
...
def insertSummaries(db):
	...
	for jName in jzimSummaries:
		...
		profileurl, loccountrycode) VALUES ('%s', '%s', %d, '%s', '%s', '%d', '%d', '%d', '%s', '0', %d, '%s',\
		 '%s')" % (myId, avatar, communityvisibilitystate, avatarmedium, personaname, personastate, 
				profilestate, lastlogoff, avatarfull, personastateflags, profileurl, loccountrycode)
		try:
			cursor.execute(sql)
			db.commit()
		except:
...
def insertAchievements(db):
	...
	for jName in jzimAchievements:
		...
		for acv in achievements:
			...
			VALUES (%d, '%s', %d, '%s', %d, '%s', '%s')" % (int(myId), achi_name, defaultvalue, displayname,
			hidden, icon, icongray)
